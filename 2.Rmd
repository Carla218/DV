...
<div style="background-color:#F0F0F0">
&emsp;<i class="fas fa-comment-dots"></i>
Answer:The gene is expressed the most in lymphocytes. It is highly expressed in all tissues.
...
<div style="background-color:#F0F0F0">
&emsp;<i class="fas fa-comment-dots"></i>
Answer: In the mitochondrial genome
...
```{r}
ggplot(data, aes(gene_type, log10(median_expression))) +
  geom_boxplot()
```
---
## Biological example
...
- Which tissue has more expressed genes?
  ...
  - What variables do we want in the `x`-axis and in the `y`-axis?
  x: tissue
  We don't put y, as it is the count of the observations.
...
ggplot(data, aes(tissue,log10(median_expression))) +
  geom_boxplot() +
  geom_point(data=data_gene, color="red")
```
...
ggplot(data_brain, aes(gene_chr, median_expression, colour=median_expression>20000)) +
  geom_point() +
  scale_color_manual(values=c("orange2","cadetblue3"))
```
...
<div style="background-color:#F0F0F0">
&emsp;<i class="fas fa-comment-dots"></i>
Answer: ggtree, gganatogram, ggseqlogo, gggenes, gggenomes, UpSetR, mraR
